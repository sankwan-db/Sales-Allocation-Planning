        ...
        <div className="flex flex-wrap items-center justify-between gap-y-3">
          <div className="flex flex-col">
            <div className="flex items-center space-x-2 font-label-sm text-label-sm text-on-surface-variant mb-0.5">
              ...
              <span className="text-on-surface font-semibold">ใบเสนอราคา (Quotation)</span>
            </div>
            <h1 className="font-headline-lg text-headline-lg text-on-surface tracking-tight">ใบเสนอราคา (Quotation)</h1>
          </div>
      ...
      <section className="bg-surface-container-lowest border-b border-outline-variant px-space-2xl py-2 flex flex-wrap items-center justify-between gap-3 shrink-0">
        <div className="flex items-center gap-3">
          <div className="relative w-64">
            ...
            <input className="w-full h-8 pl-8 pr-3 text-body-sm bg-surface border border-outline-variant rounded focus:border-primary focus:ring-1 focus:ring-primary placeholder:text-outline" placeholder="ค้นหาเลขที่ QT, ชื่อลูกค้า..." type="text" />
          </div>
          <select className="h-8 py-0 pl-2 pr-7 bg-surface border border-outline-variant rounded font-body-sm text-body-sm text-on-surface">
            ...
            <option>รออนุมัติ (Pending)</option>
            <option>อนุมัติแล้ว (Approved)</option>
            <option>หมดอายุ (Expired)</option>
          </select>
        </div>
        <div className="flex items-center gap-2">
        ...
        <table className="w-full text-left border-collapse min-w-[1000px]">
          <thead className="sticky top-0 z-20 bg-surface-container border-b border-outline shadow-sm">
            <tr className="h-9 font-badge-caps text-badge-caps uppercase tracking-wider text-on-surface-variant">
              ...
              <th className="px-4 font-semibold border-r border-outline-variant w-32">เลขที่เอกสาร (QT)</th>
              <th className="px-4 font-semibold border-r border-outline-variant w-32">วันที่เอกสาร</th>
              <th className="px-4 font-semibold border-r border-outline-variant">ชื่อลูกค้า (Customer)</th>
              <th className="px-4 font-semibold border-r border-outline-variant w-40 text-right">มูลค่ารวม (THB)</th>
          ...
          <tbody className="divide-y divide-outline-variant font-body-sm text-body-sm">
            {[
              { id: 'QT-2510-001', date: '17 ต.ค. 2025', customer: 'บริษัท ไทยฟู้ดส์ โลจิสติกส์ จำกัด', amount: '1,250,000.00', status: 'Approved', statusClass: 'bg-emerald-100 text-emerald-800 border-emerald-200' },
              { id: 'QT-2510-002', date: '16 ต.ค. 2025', customer: 'Lotus\'s (Head Office)', amount: '3,420,500.00', status: 'Pending', statusClass: 'bg-blue-100 text-blue-800 border-blue-200' },
              { id: 'QT-2510-003', date: '15 ต.ค. 2025', customer: 'Big C Supercenter', amount: '890,000.00', status: 'Warning', statusClass: 'bg-amber-100 text-amber-800 border-amber-200', label: 'Revise' },
              { id: 'QT-2510-004', date: '10 ต.ค. 2025', customer: 'Makro Cash & Carry', amount: '4,150,000.00', status: 'Closed', statusClass: 'bg-gray-100 text-gray-800 border-gray-200' },
              { id: 'QT-2510-005', date: '05 ต.ค. 2025', customer: 'ร้านอาหารเครือข่ายภาคอีสาน', amount: '450,000.00', status: 'Critical', statusClass: 'bg-red-100 text-red-800 border-red-200', label: 'Rejected' },
            ].map((row, i) => (
              <tr key={i} className="h-10 hover:bg-surface-container-low transition-colors group">
                ...
                <td className="px-4 border-r border-outline-variant/50 text-on-surface-variant">{row.date}</td>
